...
export default function Page({ }: Props) {
    ...
    useEffect(() => {
        const fetchPlace = async () => {
            try {
                const place = await GetPlaceByID(closestEvent?.placeID) as Place;
                setEventPlace(place);
            } catch (error) {
    ...
    return (
        <div className='min-h-full'>
            ...
            <div className="bg-white text-black py-2">
                {closestEvent && (
                    <>
                        <p className={'text-lg p-2 font-medium ' + roboto.className}>{closestEvent.description}</p>
                        <InfoItem icon={<FaMapLocationDot className='text-6xl text-secondary-300' />} label="Place" value={eventPlace?.name} />
                        <InfoItem icon={<FaCalendarDays className='text-6xl text-secondary-300' />} label="Dates" value={`From ${formatDate(closestEvent.startDate)} to ${formatDate(closestEvent.endDate)}`} />
                    </>
                )}
            </div>
